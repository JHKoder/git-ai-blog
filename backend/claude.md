...
| AI 개선 수락 후 재요청 시 Claude 400 Bad Request | 수락 시 `post.content`에 `> 이 글은 {모델명}이 작성을 도왔습니다.` 줄이 잔존 → 재요청마다 누적 → 프롬프트 비대화 → 컨텍스트 한도 초과 | (1) `saveResult()` 저장 전 `removeAiAuthorLine()`으로 저자 줄 제거. (2) content 600K chars 초과 시 앞부분 트리밍 (`truncateIfTooLong`) |
