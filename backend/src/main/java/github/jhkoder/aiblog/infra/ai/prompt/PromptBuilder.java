...
public class PromptBuilder {
    ...
    private String getInstruction(ContentType contentType) {
        String base = """
                ...
                - 다이어그램 작성 규칙 (가독성 최우선):
                  1. 다이어그램 바로 위에 한 줄 핵심 요약 제시. 예: `TPS 급증 → Deadlock → Pool 고갈 → 503`
                  2. 노드가 5개 이상이면 반드시 `subgraph`로 단계 그룹화. 예: [트래픽 변화] → [비즈니스 충돌] → [DB 내부 현상] → [시스템 영향] → [최종 결과]
                  3. `flowchart LR`(가로) + `subgraph` 조합 사용. `flowchart TD` 단독 나열 금지.
                  4. 노드 텍스트는 명사형 키워드 위주, 한 박스에 한 개 의미만.
